...
namespace GameEngine {
	...
	void SoundSystem::AddSoundComponent(Sound* newSound)
	{
		Component* comp = getComponent(newSound->m_handle);

		if (comp == nullptr)
		{
			std::shared_ptr<Sound> newSystem(newSound);
	...
	Sound* SoundSystem::getSoundComponent(std::string handle)
	{
		for (mapiter = map_components.begin(); mapiter != map_components.end(); mapiter++) {

			if (mapiter->first == handle) {
				return (Sound*)(mapiter->second.get());
			}
		}
	...
	void SoundSystem::AddSound(std::string filename, std::string instance_name)
	{
		...
		if (foo->Init())
		{
			Sound* comp = this->getSoundComponent(instance_name);
			if (comp == nullptr)
			{
				std::shared_ptr<Sound> newSystem(foo);
	...
	bool SoundSystem::SetSource(std::string name, float pos[3], float vel[3], bool Looping)
	{
		for (mapiter = map_components.begin(); mapiter != map_components.end(); mapiter++) {
			...
			if (mapiter->first == name) {
				Sound* foo = (Sound*)(mapiter->second.get());
